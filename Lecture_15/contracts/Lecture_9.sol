...
abstract contract Herbivore is Animal, HasName{
    ...
    function eat(string memory food) view override virtual public onlyPlant(food) returns(string memory){
...
abstract contract Carnivore is Animal, HasName{ 
    ...
    function eat(string memory food) view override onlyMeat(food) virtual public returns(string memory){
        return super.eat(food);
    }
}
